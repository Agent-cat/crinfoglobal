...
export function sanitizeArticleInput(data) {
    ...
    if (data.abstract)
        sanitized.abstract = sanitizeText(data.abstract);
    if (data.keywords)
        sanitized.keywords = sanitizeText(data.keywords);
    if (data.doi)
        sanitized.doi = sanitizeDoi(data.doi);
    if (data.conflictOfInterest)
        sanitized.conflictOfInterest = sanitizeText(data.conflictOfInterest);
    if (data.fundingInfo)
        sanitized.fundingInfo = sanitizeText(data.fundingInfo);
    if (data.dataAvailability)
        sanitized.dataAvailability = sanitizeText(data.dataAvailability);
    if (data.articleType)
        sanitized.articleType = sanitizeText(data.articleType);
    // Sanitize authors
    if (data.authors) {
        try {
            const authors = typeof data.authors === 'string' ? JSON.parse(data.authors) : data.authors;
            if (Array.isArray(authors)) {
                sanitized.authors = authors.map((author) => ({
