...
export const sendEditorNotificationEmail = async (articleData, pdfPath, editorEmails) => {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f0f8f0; padding: 15px; border-radius: 5px; margin: 20px 0;">
          ...
          <ul style="margin-bottom: 0;">
            ${articleData.authorsJson.map((author) => `<li>${author.name} (${author.email})${author.affiliation ? ` - ${author.affiliation}` : ''}</li>`).join('')}
          </ul>
        </div>
        ` : ''}
...
export const sendUserSubmissionConfirmationEmail = async (userEmail, articleData) => {
    const mailOptions = {
        from: process.env.EMAIL_USER,
        to: userEmail,
